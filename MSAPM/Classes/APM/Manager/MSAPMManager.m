...
//LeakEyeDelegate
@interface MSAPMManager ()<CrashEyeDelegate,ANREyeDelegate,TDFPSMonitorDelegate,MSCallTraceDelegate>
{
    ANREye *anrEye;
    //开始时间
    long long startTime;
...
//是开启方法调用监控
@property(nonatomic,assign)BOOL isStartObjectMethodsMonitor;
//基本性能数据定时器间隔
...
//是否正在写入数据
@property(nonatomic,assign)BOOL isObjectWrite;
@end
...
//写入object沙盒队列(消费者模式队列)
static inline dispatch_queue_t td_log_IO_Objectqueue() {
    static dispatch_queue_t td_log_IO_Objectqueue;
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        td_log_IO_Objectqueue = dispatch_queue_create("com.tuandaiguo.td_log_IO_Objectqueue", NULL);
    });
    return td_log_IO_Objectqueue;
}
//写入Object内存的队列(生产者模式队列)
static inline dispatch_queue_t td_log_IO_ObjectProducequeue() {
    static dispatch_queue_t td_log_IO_ObjectProducequeue;
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        ...
        obs = [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidFinishLaunchingNotification
                                                            ...
                                                            usingBlock:^(NSNotification *note) {
                                                                dispatch_async(dispatch_get_main_queue(), ^{
                                                                    ...
                                                                    NSString *appStartupTime =  [[MSAPMManager sharedInstance] getStringAppStartupTime:MachTimeToSeconds(applicationRespondedTime - loadTime)];
                                                                    [[MSAPMManager sharedInstance] normalDataStrAppendwith:appStartupTime];
                                                                });
...
{
    static MSAPMManager * instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[MSAPMManager alloc] init];
    });
    return instance;
}
- (instancetype)init
{
    ...
    if (self) {
        ...
        objectCondition = [[NSCondition alloc]init];
    }
    return self;
}
...
- (void)setIsStartNetworkMonitor:(BOOL)isStartNetworkMonitor {
    ...
    if (isStartNetworkMonitor) {
        ...
        //        [MSAPMSDK startNetworkOutputBlock:^(NSString * _Nonnull networkInfo) {
        //            
...
- (void)setIsStartObjectMethodsMonitor:(BOOL)isStartObjectMethodsMonitor {
    _isStartObjectMethodsMonitor = isStartObjectMethodsMonitor;
...
//写入Object沙盒性能数据
- (void)writeObjectPerformanceDadaToDisk {
    //是否开始写入
    if (self.isObjectWrite) {//开始写入 就返回 保证就一次调用这里
        return;
    }
    self.isObjectWrite = YES;
    __weak typeof(self) weakSelf = self;
    dispatch_async(td_log_IO_Objectqueue(), ^{//消费者模式
        
        while (1) {
            //等待通知  等待唤醒
            dispatch_semaphore_wait(self ->objectSemaphore,DISPATCH_TIME_FOREVER);
            //锁住
            [self ->objectCondition lock];
            //写入沙盒中
            NSData *normalData = [weakSelf.normalObjectDataStr dataUsingEncoding:NSUTF8StringEncoding];
            [weakSelf writeObjectToFileWith:normalData];
            //解锁
            [self ->objectCondition unlock];
        }
...
// 方法文件写入操作
- (void)writeObjectToFileWith:(NSData *)data {
    NSString *path = [self createFilePath];
    NSString *fileDicPath = [path stringByAppendingPathComponent:@"appObjectLogIOS.txt"];
    // 4.创建文件对接对象
    NSFileHandle *handle = [NSFileHandle fileHandleForUpdatingAtPath:fileDicPath];
    if (handle == nil) {
        [[NSData new] writeToFile:fileDicPath atomically:YES];
...
//拼接Object数据
- (void)normalObjectDataStrAppendwith:(NSString*)str {
    __weak typeof(self) weakSelf = self;
    dispatch_async(td_log_IO_ObjectProducequeue(), ^{//生产者队列
//        
//        if (str) {
//            //锁住
//            [self ->objectCondition lock];
//            //拼接数据
//            [weakSelf.normalObjectDataStr appendString:str];
//            //解锁
//            [self ->objectCondition unlock];
//            //通知 唤醒写入数据
//            dispatch_semaphore_signal(self ->objectSemaphore);
//        }
...
- (void)clearObjectCache {
    self.normalObjectDataStr = [[NSMutableString alloc] initWithString:@""];
    //清空流量
    //  [[TDNetFlowDataSource shareInstance] clear];
}
//获取app基本信息数据
...
- (NSMutableString *)normalObjectDataStr {
    
    if (_normalObjectDataStr) {
        return _normalObjectDataStr;
    }
    _normalObjectDataStr = [[NSMutableString alloc]init];
    return _normalObjectDataStr;
}
...
#pragma mark - MSCallTraceDelegate
- (void)callTraceObjectMethodCallInformation:(NSString *)objectMethods {
    NSLog(@"------objec=");
    [self normalObjectDataStrAppendwith:objectMethods];
}
