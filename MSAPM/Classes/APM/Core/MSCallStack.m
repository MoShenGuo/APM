...
#import "MSCallStack.h"

//为通用回溯设计结构支持栈地址由小到大，地址里存储上个栈指针的地址
typedef struct MSStackFrame {
    const struct MSStackFrame *const previous;
    const uintptr_t return_address;
} MSStackFrame;

//存储 thread 信息的结构体
typedef struct MSThreadInfoStruct {
    double cpuUsage;
    integer_t userTime;
} MSThreadInfoStruct;

static mach_port_t _msMainThreadId;
@implementation MSCallStack

+ (void)load {
    _msMainThreadId = mach_thread_self();
}

#pragma mark - Interface
+ (NSString *)callStackWithType:(MSCallStackType)type {
    if (type == MSCallStackTypeAll) {
        ...
        for (int i = 0; i < thread_count; i++) {
            //当前执行的指令
            [reStr appendString:msStackOfThread(threads[i])];
        }
        //task info
        NSString *memStr = @"";
        ...
        return [reStr copy];
    } else if (type == MSCallStackTypeMain) {
        //主线程
        NSString *reStr = msStackOfThread((thread_t)_msMainThreadId);
        assert(vm_deallocate(mach_task_self(), (vm_address_t)_msMainThreadId, 1 * sizeof(thread_t)) == KERN_SUCCESS);
        NSLog(@"%@",reStr);
        return [reStr copy];
    } else {
        //当前线程
        ...
        [nsthread setName:originName];
        reStr = msStackOfThread(mach_thread_self());
        NSLog(@"%@",reStr);
        return [reStr copy];
    }
    return @"";
...
//当前栈帧结构
NSString *msStackOfThread(thread_t thread) {
    //thread info
    MSThreadInfoStruct threadInfoSt = {0};
    ...
    mach_msg_type_number_t state_count = msThreadStateCountByCPU();
    kern_return_t kr = thread_get_state(thread, msThreadStateByCPU(), (thread_state_t)&machineContext.__ss, &state_count);
    if (kr != KERN_SUCCESS) {
        return [NSString stringWithFormat:@"Fail get thread: %u", thread];
    }
    //通过指令指针来获取当前指令地址
    const uintptr_t instructionAddress = msMachInstructionPointerByCPU(&machineContext);
    buffer[i] = instructionAddress;
    ++i;
    
    //
    uintptr_t linkRegisterPointer = msMachThreadGetLinkRegisterPointerByCPU(&machineContext);
    ...
    MSStackFrame stackFrame = {0};
    //通过栈基址指针获取当前栈帧地址
    const uintptr_t framePointer = msMachStackBasePointerByCPU(&machineContext);
    if (framePointer == 0 || msMemCopySafely((void *)framePointer, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
        return @"Fail frame pointer";
    }
    for (; i < 32; i++) {
        buffer[i] = stackFrame.return_address;
        if (buffer[i] == 0 || stackFrame.previous == 0 || msMemCopySafely(stackFrame.previous, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
            break;
        }
    }
    ...
    msSymbolicate(buffer, symbolicated, stackLength, 0);
    for (int i = 0; i < stackLength; ++i) {
        [reStr appendFormat:@"%@", msOutputLog(i, buffer[i], &symbolicated[i])];
    }
    [reStr appendFormat:@"\n"];
    return reStr;
}
...
NSString* msOutputLog(const int entryNum, const uintptr_t address, const Dl_info* const dlInfo) {
...
kern_return_t msMemCopySafely(const void *const src, void *const dst, const size_t byteSize) {
    vm_size_t bytesCopied = 0;
    return vm_read_overwrite(mach_task_self(), (vm_address_t)src, (vm_size_t)byteSize, (vm_address_t)dst, &bytesCopied);
}

#pragma mark - Symbolicate
void msSymbolicate(const uintptr_t* const stackBuffer, Dl_info* const symbolsBuffer, const int stackLength, const int skippedEntries) {
    int i = 0;
    if (!skippedEntries && i < stackLength) {
        msDladdr(stackBuffer[i], &symbolsBuffer[i]);
        i++;
    }
    for (; i < stackLength; i++) {
        msDladdr(msInstructionAddressByCPU(stackBuffer[i]), &symbolsBuffer[i]);
    }
}
bool msDladdr(const uintptr_t address, Dl_info* const info) {
    info->dli_fname = NULL;
    info->dli_fbase = NULL;
    info->dli_sname = NULL;
    info->dli_saddr = NULL;
    //根据地址获取是哪个 image
    const uint32_t idx = msDyldImageIndexFromAddress(address);
    if (idx == UINT_MAX) {
        return false;
    }
    /*
    ...
    const uintptr_t segmentBase = msSegmentBaseOfImageIndex(idx) + imageVMAddressSlide;
    if (segmentBase == 0) {
        return false;
    }
    //
    ...
    uintptr_t cmdPointer = msCmdFirstPointerFromMachHeader(machHeader);
    if (cmdPointer == 0) {
        return false;
    }
...
uint32_t msDyldImageIndexFromAddress(const uintptr_t address) {
...
uintptr_t msCmdFirstPointerFromMachHeader(const struct mach_header* const machHeader) {
...
uintptr_t msSegmentBaseOfImageIndex(const uint32_t idx) {
    const struct mach_header* machHeader = _dyld_get_image_header(idx);
    //查找 segment command 返回 image 的地址
    uintptr_t cmdPtr = msCmdFirstPointerFromMachHeader(machHeader);
    if (cmdPtr == 0) {
        return 0;
    }
 ...
 */
uintptr_t msMachStackBasePointerByCPU(mcontext_t const machineContext) {
...
uintptr_t msMachInstructionPointerByCPU(mcontext_t const machineContext) {
...
uintptr_t msInstructionAddressByCPU(const uintptr_t address) {
#if defined(__arm64__)
...
mach_msg_type_number_t msThreadStateCountByCPU() {
#if defined(__arm64__)
 ...
 */
thread_state_flavor_t msThreadStateByCPU() {
#if defined(__arm64__)
...
uintptr_t msMachThreadGetLinkRegisterPointerByCPU(mcontext_t const machineContext) {
#if defined(__i386__)
    return 0;
#elif defined(__x86_64__)
    return 0;
